...
2 novembre tombant un dimanche : n'était pas détecté comme anomalie par l'audit (le contenu de la Commémoraison est complet, donc rien ne « manquait »), mais le mauvais office gagnait. À la Messe, la Commémoraison l'emporte à raison sur un dimanche ordinaire (son rang la place déjà au-dessus). Mais à l'Office (Laudes/Vêpres/Lectures/Sexte-Tierce-None/Complies), c'est l'inverse : faute de règle spécifique, le même calcul de précédence faisait gagner la Commémoraison partout, alors que l'Office du dimanche doit primer (l'Office des défunts restant une option pastorale, pas la valeur par défaut). Ajout de `detectOfficeCelebrations()` dans `office_detection.dart` — un wrapper utilisé par les 5 détecteurs d'office (pas par `massDetection`, qui reste inchangé) qui rétrograde la précédence effective de la Commémoraison sous celle du dimanche quand les deux coïncident.
