import '../../classes/compline_class.dart';

final Map<String, Compline> christmasTimeCompline = {
  "saturday": Compline(
    complinePsalm1Antiphon1:
        "Nos yeux ont vu le salut: qui peut nous ravir notre joie?",
    complinePsalm2Antiphon1: "Sur le pays de l'ombre, une lumière s'est levée.",
    complineEvangelicAntiphon:
        "J'ai vu de mes yeux le Sauveur: lumière des peuples.",
  ),
  "sunday": Compline(
    complinePsalm1Antiphon1:
        "Il suffit que tu ouvres les yeux, tu verras le salut.",
    complinePsalm1Antiphon2: " ",
    complineEvangelicAntiphon:
        "J'ai vu de mes yeux le Sauveur: lumière des peuples.",
  ),
  "monday": Compline(
    complinePsalm1Antiphon1:
        "Tous les peuples viendront t'adorer, car tu fais des merveilles.",
  ),
  "tuesday": Compline(
    complinePsalm1Antiphon1:
        "Sur l'œuvre de tes mains, je médite, je tends le mains vers toi.",
    complinePsalm1Antiphon2: " ",
  ),
  "wednesday": Compline(
    complinePsalm1Antiphon1:
        "Plus q'un veilleur ne guette l'aurore, attends le Seigneur.",
    complinePsalm1Antiphon2: " ",
  ),
  "thursday": Compline(
    complinePsalm1Antiphon1: "J'ai dit au Seigneur: Tu es mon Dieu!",
    complinePsalm1Antiphon2: " ",
  ),
  "friday": Compline(
    complinePsalm1Antiphon1: "Seigneur, mon Dieu et mon salut!",
    complinePsalm1Antiphon2: " ",
  ),
};
