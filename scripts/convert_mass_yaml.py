...
def _convert_reading(c):
    ...
    if c.get('shortReadingRef') is not None:
        result['shortReadingRef'] = _lit(_merge_book_ref(c.get('book'), c['shortReadingRef']))
    if c.get('shortReadingContent') is not None:
        result['shortReadingContent'] = _lit(c['shortReadingContent'])
    return result
...
def main():
    base = Path(__file__).parent / 'assets' / 'mass_missal'
    if len(sys.argv) == 3:
        source_dir = base / sys.argv[1]
        target_dir = base / sys.argv[2]
    else:
        source_dir = base / 'ferial_days'
        target_dir = base / 'new'
